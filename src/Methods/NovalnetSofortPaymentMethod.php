...
class NovalnetSofortPaymentMethod extends PaymentMethodService
{
    ...
    {
        return (bool)(($this->configRepository->get('Novalnet.sofort_payment_active') == 'true') && is_numeric($this->paymentHelper->getNovalnetConfig('vendor_id')) && !empty($this->paymentHelper->getNovalnetConfig('auth_code')) && is_numeric($this->paymentHelper->getNovalnetConfig('product_id')) && is_numeric($this->paymentHelper->getNovalnetConfig('tariff_id')) && !empty($this->paymentHelper->getNovalnetConfig('access_key')));
    }
    ...
    {   
        ...
        if(empty($name))
        {
            $name = $this->paymentHelper->getTranslatedText('sofort_payment_frontend_name');
        }
        return $name;
    }
    ...
    {
        $description = trim($this->configRepository->get('Novalnet.sofort_description'));
        if(empty($description))
        {
            $description = $this->paymentHelper->getTranslatedText('sofort_payment_description');
        }
        return $description;
    }
